...
SYSTEM_PROMPT = (
    ...
    "Return ONLY a JSON object with key 'references' whose value is an array of objects with fields: provider (always 'openai'), id (best persistent id), doi, title, abstract (1-2 sentence summary if not available), year, venue, authors, url, is_open_access. "
...
def process_side(client: OpenAI, model: str, claims_path: str, out_path: str, limit: int, per_claim: int, timeout: float, sleep_s: float) -> None:
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    process_side(client, model=args.model, claims_path=dem_claims, out_path=dem_out, limit=args.limit, per_claim=args.per_claim, timeout=args.timeout, sleep_s=args.sleep)
    logger.info("Processing Republican claims")
    process_side(client, model=args.model, claims_path=rep_claims, out_path=rep_out, limit=args.limit, per_claim=args.per_claim, timeout=args.timeout, sleep_s=args.sleep)
    logger.info("Done")
    return 0
