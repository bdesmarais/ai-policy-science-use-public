...
Two properties show the pipeline is reproducible and not tied to one implementation. \emph{Reproducible
retrieval.} Replacing the generative reference retrieval of earlier pipelines with retrieval from the
open OpenAlex index returns only real, DOI-bearing papers, eliminating the citation-fabrication failure
mode of asking a model to name sources. OpenAlex's keyword relevance search is broader and less
claim-targeted than bespoke generative retrieval, so a larger share of retrieved papers are off-topic
for the specific claim and are judged \textsc{silent}; naive support rates fall accordingly. This makes
explicit a general point: the measured \emph{level} depends on retrieval relevance as well as on the
judge, which is why the trustworthy quantity is the validated-judge estimate on a fixed corpus rather
than any single naive rate. The partisan \emph{ordering}, by contrast, is preserved under the LLM judges
on the OpenAlex references as well.\pending{[verify ordering once panel-on-OpenAlex completes]}
\emph{Domain transfer.} The pipeline's front end applies unchanged
